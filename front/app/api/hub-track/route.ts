...
/** The same cap the hub enforces. */
const MAX_EVENTS = 50;

/**
 * The visit cookie. Half an hour of inactivity ends it, which is the usual
 * session window, and it doesn't survive closing the browser.
 *
 * It carries a random identifier and nothing else: no identity, no history.
 * It's the only thing that stops five pages from counting as five visits.
 */
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // caller why its payload was dropped only helps whoever is probing it.
  ...
  // The identifier is issued here and not in the page: as an httpOnly cookie
  ...
  noContent.cookies.set(SESSION_COOKIE, sessionId, {
    ...
    path: "/",
    // Renewed on every event, so the visit ends after half an hour of silence
